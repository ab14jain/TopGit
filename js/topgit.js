...
$(document).ready(function () {
    ...
    var jsonLanguage = $.parseJSON(languages);
    $("#search").autocomplete({
        minLength: 0,
        source: jsonLanguage,
    ...
    $("#star-slider").slider({
        ...
        slide: function (event, ui) {
            starMin = ui.values[0];
            starMax = ui.values[1];
            $("#range-level").html(ui.values[0] + " - " + ui.values[1]);
            url = "https://api.github.com/search/repositories?q=stars:>=" + starMin + "+stars:<" + starMax + "+language:" + selectedLanguage + "&sort=stars&order=desc";

            if (selectedLanguage) getData(url);
        }
    });
    ...
    $("#rate-limit").slider({
        ...
        values: [1, 10],
        slide: function (event, ui) {
            $("#rate-level").html(ui.values[0] + " - " + ui.values[1]);
        }
    });
...
function getData(url) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $(".result-count").html("");
            $(".result-count").append("<p class='result-msg'>We\'ve found <strong id='count'>" + data.items.length + "</strong> results in <strong id='language'>" + selectedLanguage + "..</strong></p>")
            ...
            $.each(data.items, function (index, value) {
                ...
                var htmlurl = data.items[index].html_url;
                var language = data.items[index].language;
                ...
                var temp_html = "<div class='result'>" +
                                   "<div class='icon'></div>" +
                                   "<div id='title'><a id='title-link' target = '_blank' href= " + htmlurl + ">" + fullName + "</a></div>" +
                                   "<div id='planguage'>" +
                                   "<p>Language: " + language + "  | " + description + "  </p>" +
                                   "</div></div>";
